...
function GoogleButtonShell({ label, loading, disabled, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function GoogleSignInButtonActive({ label, loading, disabled, onSuccess, onError }) {
  ...
  if (!clientId) {
    return (
      <GoogleButtonShell
        ...
        onClick={() =>
          onError?.(
            'Google sign-in is not configured. Add VITE_GOOGLE_CLIENT_ID to your client .env file.'
          )
        }
      />
    )
